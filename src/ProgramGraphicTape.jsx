/**
 * COMPONENTE: ProgramGraphicTape.jsx
 * 
 * DESCRIPCIÓN:
 ...
 * - Visualización de métricas clave (contenido total, programas  grabados)
...
const ProgramGraphicTape = () => {
    ...
    const [filters, setFilters] = useState({
        ...
        network: '',
        status: '',
        ltsa: 'Tape', // para mostrar programas grabados por defecto
        search: '',
        showCode: ''
    });
            ...
            <Row className="dashboard-header align-items-center mb-4">
                <Col md={8}>
                    <Stack direction="horizontal" gap={3} className="align-items-center">
                        <div>
                            <h3 className="mb-1">Programs Tape Report Graphic  </h3>
...
*/}
                    ...
                    <Card id="time-slots-chart-container" ref={timeSlotsRef} className="chart-card">
                        <Card.Header className="chart-header">
                            <div className="d-flex justify-content-between align-items-center flex-wrap">
                                <div>
                                    <FiClock className="me-2" />Tape Programs Time
            ...
            {/* Gráfico de programas Tape en 7 días */}
            ...
            <Row className="mb-4">
                <Col xl={12}>
                    <Card className="calendar-matrix-card">
                        <Card.Header className="bg-primary text-white">
                            <FiCalendar className="me-2" />
                            Show Code Appearance Calendar Matrix
                        </Card.Header>
                        <Card.Body>
                            <div style={{ height: '600px', overflow: 'auto' }}>
                                <ResponsiveContainer width="100%" height={Math.max(400, filteredData.length * 20)}>
                                    <HeatMapChart
                                        data={useMemo(() => {
                                            // Agrupar por Show Code
                                            const showCodeMap = filteredData.reduce((acc, program) => {
                                                const showCode = program["Show Code"];
                                                if (!acc[showCode]) {
                                                    acc[showCode] = {
                                                        name: showCode,
                                                ...
                                                const programDate = new Date(program.start).toLocaleDateString('es-ES');
                                                acc[showCode].days[programDate] = (acc[showCode].days[programDate] || 0) + 1;

                                                return acc;
                                            }, {});

                                            // Convertir a formato para el heatmap
                                            return Object.entries(showCodeMap).map(([showCode, data]) => ({
                                                showCode,
                                                title: data.title,
                                                ...Object.entries(data.days).reduce((acc, [date, count]) => {
                                                    acc[date] = count;
                                                    return acc;
                                                }, {})
                                            }));
                                    ...
                                    >
                                        ...
                                        <Tooltip
                                            content={({ active, payload }) => {
                                                if (active && payload && payload.length) {
                                                    ...
                                                    return (
                                                        <div className="custom-tooltip p-3 bg-white border rounded shadow">
                                                            <p className="fw-bold mb-1">{data.showCode}</p>
                                                            <p className="mb-1"><strong>Title:</strong> {data.title}</p>
                                                            <p className="mb-1"><strong>Date:</strong> {new Date(date).toLocaleDateString('es-ES', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
                                                            <p className="mb-0"><strong>Appearances:</strong> {payload[0].value}</p>
                                                        </div>
                                                    );
                                                }
                                                return null;
                                        ...
                                        >
                                            {filteredData.map((entry, index) => (
                                                <Cell
                                                    key={`cell-${index}`}
                                                    fill={new Date(entry.start) < new Date(dateRange[0].startDate) ? '#2E86AB' : '#28a745'}
                                                />
                                            ))}
                                        </Heatmap>
                                        <Legend>
                                            <div className="d-flex justify-content-center mt-3">
                                                <Stack direction="horizontal" gap={3}>
                                                    <div className="d-flex align-items-center">
                                                        <div className="color-box" style={{ backgroundColor: '#2E86AB', width: '20px', height: '20px' }}></div>
                                                        <span className="ms-2">Previously Recorded</span>
                                                    </div>
                                                    <div className="d-flex align-items-center">
                                                        <div className="color-box" style={{ backgroundColor: '#28a745', width: '20px', height: '20px' }}></div>
                                                        <span className="ms-2">Current Period</span>
                                                    </div>
                                                </Stack>
                                            </div>
                                        </Legend>
                                ...
                                <tbody>
                                    {filteredData.map((program, index) => (
                                        ...
                                        >
                                            ...
                                            <td>{program.Duration}</td>
                                            <td>
                                                <Badge bg={program.LTSA === 'Live' ? 'danger' || program.LTSA === 'Tape' : 'secondary'}>
                                                    {program.LTSA}
                                                </Badge>
                                            </td>
...
export default ProgramGraphicTape;