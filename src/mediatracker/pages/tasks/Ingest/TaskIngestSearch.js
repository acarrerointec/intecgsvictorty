...
const TaskIngestSearch = ({
  ...
  const adaptarGroupsChannels = (datos) => {
    ...
    datos.forEach((d) =>{
    //TODO:  hay que cambiar el value por canal_grupo y cambiar la logica dentro de 
    opciones.push({ value: numberOrder(d.feeds).join(","), label: d.descrip })}
    );
    return opciones;
  };
  ...
  const adaptarGroupsTypes = (datos) => {
    const opciones = [];
    datos.forEach((d) =>
      opciones.push({disabled:false, value: d.grupoId, label: d.descrip, groupData: d.deta?.map(t => getTipoNameUnified(t)) })
    );
    return opciones;
  };
  ...
  useEffect(() => {
    localStorage.setItem("saveSearch", JSON.stringify(saveSearchTempo));
  }, [refreshSavedFilters]);
  ...
  const formatGroup = (group, prevGroups, feed) => {
    ...
    let arrayFeed = [...feed];
    const groupsToDelete = prevGroups?.filter((pg) => !group.some(g => g.label == pg.label));
    if (group) {
      group.forEach((item) => {
